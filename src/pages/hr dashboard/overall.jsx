  ...
  const Overall = ({ departments, terms, faculty }) => {
        ...
        await getAllUsers(
          ...
          (response) => {
            ...
            departments_.forEach(d => {
              ...
              const clearedMembers = members.filter(m => m.clearances?.filter(c => c.term.id === t.id).length > 0);
              const percentCleared = members.length === 0 ? 0 : Math.round((clearedMembers.length / members.length) * 100);
            ...
            const overallPercentCleared = overallMemberCount === 0
              ? 0
    ...
    const employeeTotal = deps.filter(d => selectedLevel === "" ? true : d.level === selectedLevel)
    .map(d => d.members.length)
      .reduce((acc, value) => acc + value, 0);

    const clearedTotal = deps.filter(d => selectedLevel === "" ? true : d.level === selectedLevel)
              ...
              {pieChartData && (
                <Col>
                  ...
                  <ListGroup className={`${styles.searchBar} w-100`}>
                    ...
                    :
                    currentRecords.filter(d => selectedLevel === "" ? true : d.level === selectedLevel)
                    .map((d) => (
